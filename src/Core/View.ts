export async function View(templatePath: string, context: Record<string, any> = {}): Promise<HTMLElement> {
	...
	html = html.replace(/{{#each\s+(\w+)}}([\s\S]*?){{\/each}}/g, (_, key, block) => {
	...
	html = html.replace(/{{#if\s+([\w.]+)}}([\s\S]*?){{\/if}}/g, (_, condition, block) => {
